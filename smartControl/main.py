...
import math
import numpy as np
import handControl.handDetector as htm
...
while True:
    ...
    if len(lmList) != 0:
        ...
        thum_middle_length = getLength(right_thumb_tip_x, right_thumb_tip_y,
                                       right_middle_finger_tip_x, right_middle_finger_tip_y
                                       )

        # 角度計算(大回りよう)
        thumb_angle = calcAngle(right_thumb_cmc_x, right_thum_cmc_y,
        ...
        ring_angle = calcAngle(right_ring_finger_mcp_x, right_ring_finger_mcp_y,
                               right_ring_finger_pip_x, right_ring_finger_pip_y,
                               right_ring_finger_tip_x, right_ring_finger_tip_y,
                               )

        pinky_angle = calcAngle(right_pinky_mcp_x, right_pinky_mcp_y,
                                right_pinky_pip_x, right_pinky_pip_y,
                                right_pinky_tip_x, right_pinky_tip_y
                                )

        # 角度計算(小回り用)
        index_angle_2 = calcAngle(right_index_finger_pip_x, right_index_finger_pip_y,
                                  right_index_finger_dip_x, right_middle_finger_dip_y,
                                  right_index_finger_tip_x, right_index_finger_tip_y,
                                  )

        middle_angle_2 = calcAngle(right_middle_finger_pip_x, right_middle_finger_pip_y,
                                   right_middle_finger_dip_x, right_middle_finger_dip_y,
                                   right_middle_finger_tip_x, right_middle_finger_tip_y
                                   )
        ring_angle_2 = calcAngle(right_ring_finger_pip_x, right_ring_finger_pip_y,
                                 right_ring_finger_dip_x, right_ring_finger_dip_y,
                                 right_ring_finger_tip_x, right_ring_finger_tip_y
                                 )
        pinky_angle_2 = calcAngle(right_pinky_pip_x, right_pinky_tip_y,
                                  right_pinky_dip_x, right_pinky_dip_y,
                                  right_pinky_tip_x, right_pinky_tip_y
                                  )

        # print(index_angle_2)
        # if index_angle_2 <= 50 and middle_angle_2 <= 50:
        #     print("test")

        # ナンバーワン
        if index_angle >= 170 and thum_ring_length <= 20 \
                and thum_middle_length <= 55 and thum_pinky_length <= 50:
            light_mode = True
        ...
        if music_mode:
            ...
            # ミュージックモードを抜ける
            if index_angle >= 170 and pinky_angle >= 170 \
                    and thum_ring_length <= 40 and thum_middle_length <= 40:
                print("これはyey")
                music_mode = False

        # 音声ボリューム
        if music_mode:
            cv2.line(img, (right_thumb_tip_x, right_thumb_tip_y), (right_index_finger_tip_x, right_index_finger_tip_y),
                     (255, 0, 255), 5)
